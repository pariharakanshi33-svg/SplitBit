...
class SettlementController {
   ...
   * Get minimized transactions for a group across all unsettled bills
   */
  async getMinimizedSettlements(req, res) {
    try {
      const { groupId } = req.params;

      // Fetch all unsettled settlements for the group
      ...
      if (unsettled.length === 0) {
        return res.json({
          message: 'No unsettled debts in this group',
          settlements: [],
          minimized: []
        });
      }
      ...
      // Resolve user names for the minimized transactions
   ...
   * Get all settlements for a user (both owed and owing)
   */
  async getUserSettlements(req, res) {
    try {
      ...
      let totalOwing = 0; // Money others owe you
